        ...
        try {
            if (isZip) {
                try (ZipFile zip = new ZipFile(tmp.toFile())) {
                    ...
                    ZipEntry match = zip.stream().filter(e -> !e.isDirectory()).filter(e -> {
                        String entryBaseName = Paths.get(e.getName()).getFileName().toString();
                        return entryBaseName.equals(expectedFileName);
                    }).findFirst().orElse(null);

                    if (match == null) {
                        throw new IOException("Could not find " + expectedFileName + " inside archive: " + url);
                    }
